...
`durations=None` 或 `durations=0` 会生成真正的零时长 layer，文件的
`have_animation=False`。正数 duration 才表示动画帧；也可以传列表混合
零时长 layer 和定时帧。为兼容旧代码，默认值仍是 `durations=1`。
...
```python
plane                                         # 匿名 optional 类型
("name", plane)                               # 命名
("name", "type", plane)                       # 命名 + 类型
{"name": "...", "type": "...", "data": plane} # dict 形式
```

dict 形式还支持完整元数据：`bits_per_sample`、`exponent_bits_per_sample`、`dim_shift`、
`alpha_premultiplied`（兼容别名 `alpha_associated`）、`spot_color=(r,g,b,a)`
和 `cfa_channel`。解码结果会返回这些字段以及每个 plane 的尺寸和指数位数。
`dim_shift` 范围为 0..3；非零值会让该通道在码流中降采样，因此不保证逐像素无损。
jxlpy 会自动保证 `ec_resampling >= 2 ** dim_shift`，显式传入冲突值时直接报错。

结构化色彩和 ICC 也可显式指定：

```python
jxl_bytes = jxlpy.encode(rgb, color_encoding="linear_srgb")
jxl_bytes = jxlpy.encode(rgb, icc_profile="display.icc")
meta = jxlpy.info(jxl_bytes)
print(meta["color_encoding"], meta["icc_profile"])
```

可编码类型：`alpha`, `depth`, `spot_color`, `selection_mask`, `black`, `cfa`, `thermal`, `optional`。
省略 `type` 时使用 `optional`；`unknown` 仅可能出现在解码元数据中，不能用于编码。
...
- **目标帧读取**：默认在目标帧完成后立即停止。`num_frames_known=False` 时 `num_frames` 只是已到达的帧数；需要精确总数时使用 `scan_all_frames=True` 或 `info()`。
- **解码资源控制**：`decode`、`decode_layer` 和 `decode_extra_channel` 支持 `threads`、`max_pixels`、`max_output_bytes`。
- **ABI 校验**：Python 导入时校验 native ABI 版本和共享结构尺寸，旧 DLL 会明确要求重新构建，不再冒险继续加载。
